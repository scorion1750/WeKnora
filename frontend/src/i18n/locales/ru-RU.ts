export default {
  ...
  integrations: {
    ...
    claw: {
      title: 'RAG Skill',
      subtitle: 'Импорт документов и гибридный поиск (вектор + ключевые слова) через REST API RAG — загрузки, URL, Markdown и поиск.',
      ...
      installCtaHint: 'Установка RAG Skill · откроется в новой вкладке',
    ...
    chrome: {
      ...
      subtitle: 'Для self-hosted RAG: вопросы в боковой панели, клиппинг страниц и Markdown-заметки в базы знаний прямо при просмотре.',
      ...
      steps: {
        ...
        port: {
          ...
          desc: 'В RAG Desktop задайте фиксированный порт API (например 37841) в API-информации.'
        },
        api: {
    ...
    api: {
      ...
      capabilityManageModelsHint: 'Управление конфигурациями моделей, учётными данными, проверками подключения и данными RAG Cloud.',
      ...
      hmacSecretDesc: 'Тот же secret, что в RAG; только для подписи JWT на backend — не отправляйте его в заголовке запроса.',
  ...
  kbSettings: {
    parser: {
      ...
      engines: {
        ...
        weknoracloud: {
          name: 'RAG Cloud',
          desc: 'Парсинг документов через RAG Cloud'
        },
        paddleocr_vl_cloud: {
  ...
  embedPublish: {
    ...
    originsHint: 'Укажите сайты, которым разрешено встраивание (A), а не адрес RAG (B). Один полный Origin на строку, например https://shop.example.com; минимум один. Поддерживается *.example.com.',
  ...
  createChat: {
    title: 'Привет, я RAG — ваши знания всегда под рукой',
  ...
  auth: {
    ...
    registerSubtitle: 'Создайте аккаунт и начните работу с RAG',
  ...
  envVarSettings: {
    ...
    description: 'Личные ключи для навыков и песочниц, а не системные или деплой-настройки RAG.',
  ...
  settings: {
    ...
    sandbox: {
      ...
      backendDescriptions: {
        ...
        docker: 'Держит долгоживущий контейнер на каждую сессию на этом хосте RAG; скрипты и файлы остаются в том же контейнере',
      },
      ...
      dockerHostRisk: 'Пустое значение или unix:// использует демон Docker на машине RAG — это равносильно root на этой машине. Только для частной одноузловой установки. Если несколько пространств делят один хост, используйте Cube или E2B. Удалённый tcp:// требует каталог TLS-сертификатов.',
      ...
      templateUntaggedHint: 'Сборки завершены, но ни одна не имеет тега default, поэтому при создании песочницы шаблон не находится. Удалите его в E2B и обновите список — RAG пересоберёт шаблон.',
      templateFailedReason: 'Сборка не удалась: {reason}',
      noTemplates: 'No templates were returned by this cluster.',
      weknoraStandardTemplate: 'Стандартный шаблон RAG',
      ...
      weknoraDesktopTemplate: 'Шаблон рабочего стола RAG',
      ...
      replaceStandardTemplateConfirm: 'Стандартный шаблон RAG будет собран заново с текущими настройками, включая DNS. Предыдущий готовый шаблон не удаляется, пока замена не станет готовой к запуску.',
      desktopTemplateProvisioning: 'Шаблон рабочего стола RAG создаётся. Обновите список чуть позже, чтобы увидеть статус.',
      ...
      replaceDesktopTemplateConfirm: 'Шаблон рабочего стола RAG будет собран заново с текущими настройками, включая DNS. Предыдущий готовый шаблон рабочего стола не удаляется, пока замена не станет готовой к запуску. Шаблон CLI не затрагивается.',
      ...
      cubeDnsServersHelp: 'Необязательно. IP nameserver, которые записываются в стандартный шаблон RAG. Пустое значение оставляет значение кластера по умолчанию (обычно 119.29.29.29). Если UDP/53 до публичных резолверов недоступен, укажите достижимые адреса из /etc/resolv.conf хоста Cube, исключая 10/8, 172.16/12 и 192.168/16. Чтобы изменение вступило в силу, на карточке шаблона нажмите «Пересобрать».',
    ...
    weknoraCloud: {
      title: 'RAG Cloud',
      description: 'Настройте учётные данные APPID и APPSECRET для RAG Cloud. Данные используются для модельных сервисов и движка парсинга документов.',
      ...
      expired: 'Учётные данные RAG Cloud истекли',
      ...
      appIdLabel: 'APPID',
      appIdDesc: 'ID приложения RAG Cloud',
      appIdPlaceholder: 'Введите APPID',
      appSecretLabel: 'APPSECRET',
      appSecretDesc: 'Секрет приложения RAG Cloud',
      ...
      modelsSection: {
        ...
        descReady: 'Зарегистрируйте четыре стандартные модели RAG Cloud для чата, поиска, реранкинга и зрения.',
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        ...
        desc: 'С этого всё начинается: загружайте документы, веб-страницы или FAQ, и RAG автоматически разберёт и проиндексирует их. Нажмите здесь, чтобы открыть базы знаний.'
      },
      welcome: {
        title: 'Добро пожаловать в RAG',
