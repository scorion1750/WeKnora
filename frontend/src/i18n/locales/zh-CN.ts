export default {
  ...
  integrations: {
    ...
    claw: {
      title: 'RAG Skill',
      subtitle: '通过 RAG REST API 导入文档并执行混合检索（向量 + 关键词）。适用于上传文件/URL/Markdown 到知识库、跨库检索与浏览知识内容。',
      ...
      installCtaHint: '安装 RAG Skill · 将在新标签页打开',
    ...
    chrome: {
      title: '知识管理助手',
      subtitle: '配合 RAG 自建服务使用：在任意网页侧边栏提问、剪藏内容、Markdown 速记，将浏览中的知识沉淀到你的知识库。',
      ...
      steps: {
        ...
        port: {
          title: '桌面版配置固定端口（推荐）',
          desc: '使用 RAG 桌面版时，在 API 信息中设置固定端口（如 37841），避免每次启动后地址变化导致插件断连。'
        },
        api: {
          title: '获取 API 凭证',
          desc: '在「设置 → API 信息」中复制 API Key 与 API 地址。'
    ...
    api: {
      ...
      capabilityManageModelsHint: '允许管理模型配置、模型凭据、模型连通性测试和 RAG Cloud 凭据。',
      ...
      signedFlowDetail: 'HMAC 密钥仅保存在 RAG 与你的可信后端，切勿放入请求或下发给浏览器。请求头里传的是用该密钥签出的 JWT 字符串（每次签发或过期后会变），不是密钥本身。JWT 须包含 sub（外部用户 ID）、tenant_id、aud=weknora、exp（有效期不超过 24 小时）。',
      ...
      hmacSecret: 'HMAC 密钥',
      hmacSecretDesc: '与 RAG 保存的同一份密钥；仅用于后端签发 JWT，不要作为请求头发送。',
  ...
  embedPublish: {
    ...
    originsHint: '填写允许嵌入的宿主网站来源（A），不是 RAG 地址（B）。每行一个完整 Origin，如 https://shop.example.com；至少一项，支持 *.example.com 子域通配',
    ...
    webhookUrlDesc: '可选。访客发消息与助手回复完成时，RAG 会向该 HTTPS 地址 POST 事件（message_sent / message_received）。',
  ...
  auth: {
    ...
    registerSubtitle: '创建账户并开始使用 RAG',
  ...
  envVarSettings: {
    title: '沙箱密钥',
    description: '给技能和沙箱用的个人密钥，不是 RAG 的系统或部署配置。',
    helpAria: '沙箱密钥说明',
  ...
  settings: {
    ...
    sandbox: {
      ...
      dockerHostRisk: '留空或 unix:// 会使用 RAG 所在机器的 Docker 守护进程，权限等同该机 root，只适合私有化单机。多套空间共用同一主机时请改用 Cube 或 E2B。远程 tcp:// 必须填写 TLS 证书目录。',
      addConfig: '添加沙箱',
      ...
      weknoraTemplateTitle: 'RAG 标准模板',
      weknoraDockerImage: 'RAG 标准镜像',
      weknoraDockerImageHint: '每次会话独占一个长驻容器，脚本、shell 命令与文件都在同一个容器内，会话结束或空闲超时后回收。',
      weknoraTemplateOverview: 'RAG 负责提供标准运行环境。连接集群后会自动发现模板，缺少标准模板时自动创建。',
      weknoraTemplateDescription: '包含 RAG 技能所需的 Python、Node.js、CLI 工具、工作目录和非 root 执行用户。',
      recommendedTag: '推荐',
      ...
      standardTemplateProvisioning: '已开始在集群中创建 RAG 标准模板，请稍后刷新查看状态',
      ...
      templateUntaggedHint: '构建已完成，但没有构建带 default 标签，创建沙箱时无法解析。请在 E2B 删除该模板，刷新后 RAG 会重新构建。',
      templateFailedReason: '构建失败：{reason}',
      noTemplates: '当前集群未返回可用模板。',
      weknoraStandardTemplate: 'RAG 标准模板',
      createStandardTemplate: '创建',
      createStandardTemplateHint: '按当前连接配置构建，包含 DNS。之后改配置可在卡片上重建。',
      weknoraDesktopTemplate: 'RAG 桌面模板',
      createDesktopTemplate: '创建',
      createDesktopTemplateHint: '从官方桌面镜像构建 XFCE 图形桌面。体积明显大于标准模板，仅在需要图形界面时创建。',
      replaceStandardTemplate: '重建',
      replaceStandardTemplateConfirm: '将用当前配置（含 DNS）重建 RAG 标准模板。新模板就绪前不会删除仍可启动的旧模板。',
      desktopTemplateProvisioning: '已开始在集群中创建 RAG 桌面模板，请稍后刷新查看状态',
      desktopTemplateReplaced: '已删除原桌面模板并开始重建，请等待状态就绪。标准（CLI）模板不受影响。',
      desktopTemplateTag: '桌面',
      replaceDesktopTemplateConfirm: '将用当前配置（含 DNS）重建 RAG 桌面模板。新模板就绪前不会删除仍可启动的旧桌面模板，也不会删除标准（CLI）模板。',
      ...
      cubeDnsServersHelp: '可选。写入 RAG 标准模板的 nameserver（须为 IP）。留空则使用集群默认（常见 119.29.29.29）。私网或云上 UDP 53 出不去时，填 Cube 宿主机 /etc/resolv.conf 里能用的地址，并避开 10/8、172.16/12、192.168/16。已有标准模板需在模板卡片上点「重建」才会生效。',
      ...
      dockerTlsCertPathHelp: 'RAG 所在主机上包含 ca.pem、cert.pem、key.pem 的目录。远程守护进程必填，证书不入库，由部署方挂载。',
      dockerIdleTtl: '空闲回收（秒）',
      dockerIdleTtlHelp: 'Docker 守护进程本身没有空闲超时。容器多久没有执行任何命令就会被 RAG 回收，会话继续时重建。留空按 1800 秒。',
    ...
    weknoraCloud: {
      ...
      modelsSection: {
        title: '云模型接入',
        descReady: '将 RAG Cloud 提供的四类标准模型注册到当前空间，用于对话、向量检索、重排序与多模态理解。',
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        title: '创建你的知识库',
        desc: '知识库是一切的起点：上传文档、网页或 FAQ，RAG 会自动解析并建立索引。点击这里进入知识库。'
      },
      welcome: {
        title: '欢迎使用 RAG',
  ...
  business: {
    home: {
      eyebrow: '业务工作台',
      title: '核电知识与科研协同',
      desc: '文档办公、科研项目与数据对接在此汇聚；需要检索与问答时，可进入 RAG 能力中心。',
      enterRag: '进入 RAG',
      bizOnline: '业务服务在线',
      bizOffline: '业务服务离线',
      phaseTitle: '一期能力完成度（1A + 2A 骨架）',
      phaseDesc: '下列为已交付能力；未勾选项属于后续阶段，不在本期范围。',
      phase: {
        skeleton: '业务微服务与业务库',
        skeletonDesc: 'FastAPI + rag_biz + JWT 校验 + Vite /biz/api 代理',
        office: '文档办公',
        officeDesc: '上传、预览、一键入库知识库；CATIA 仅登记占位',
        project: '科研项目',
        projectDesc: '项目/任务 CRUD、模板生成计划、甘特图',
        integration: '数据对接',
        integrationDesc: 'Connector CRUD、HTTP 试连、同步日志',
        later: '后续阶段（未做）',
        laterDesc: 'WPS 在线编辑、真实 CATIA 转换、院内系统适配器、达梦生产切换、出网策略页',
      },
    },
    modules: {
      office: {
        title: '文档办公',
        desc: 'Word、PDF、图片与 CATIA 轻量化模型接入，支撑写文档与资料入库。',
      },
      project: {
        title: '科研项目',
        desc: '国家项目申报、计划甘特、任务清单与进度查询全流程管理。',
      },
      integration: {
        title: '数据对接',
        desc: '对接院内其他系统数据，统一汇入业务库与知识库。',
      },
      rag: {
        title: 'RAG 能力',
        desc: '知识库、智能对话、智能体与产物，服务业务检索与问答。',
      },
    },
    common: {
      delete: '删除',
      ...
      actions: '操作',
      retry: '重试连接',
      serviceDown: '业务服务暂不可用（默认 http://127.0.0.1:8090）。请确认 biz-service 已启动后再操作。',
    },
    menuSettings: {
      title: '业务菜单',
      description: '控制左侧边栏业务入口的显示。默认关闭「数据对接」；可按空间需要开启。',
      save: '保存',
      reset: '还原',
      adminOnly: '仅空间管理员可修改业务菜单。',
      atLeastOne: '请至少启用一个业务菜单',
      items: {
        home: '工作台',
        homeDesc: '业务首页与模块入口汇总',
        office: '文档办公',
        officeDesc: '文档上传、预览、CAD 轻量化与入库',
        project: '科研项目',
        projectDesc: '项目、任务清单与甘特计划',
        integration: '数据对接',
        integrationDesc: '外部系统连接器与同步任务（默认关闭）',
      },
    },
    project: {
      create: '新建项目',
      ...
      selectHint: '请选择左侧项目',
      generatePlan: '生成计划草案',
      generatePlanDisabled: '已有任务记录，无需再生成草案',
      viewGantt: '查看甘特图',
      addTask: '添加任务',
      editTask: '编辑任务',
      deleteProject: '删除项目',
      deleteProjectConfirm: '确认删除该项目及其任务？',
      deleteTaskConfirm: '确认删除该任务？',
      tasks: '任务清单',
      tasksEmpty: '暂无任务，可添加任务或生成计划草案',
      gantt: '甘特图',
      ganttEmpty: '暂无计划条。请先为任务设置起止时间，或点击「生成计划草案」。',
      planGenerated: '已生成计划草案',
      name: '项目名称',
      code: '项目编号',
      description: '描述',
      taskTitle: '任务标题',
      assignee: '负责人',
      assigneePlaceholder: '选择空间成员',
      schedule: '执行时间',
      status: '状态',
      progress: '进度',
      start: '开始',
      ...
      statusDone: '已完成',
      statusDelayed: '延期',
      searchProject: '搜索项目名称/编号',
      searchTask: '搜索任务标题/负责人',
      searchEmpty: '无匹配结果',
      filterStatus: '按状态筛选',
    ...
    office: {
      upload: '上传文档',
      empty: '暂无文档',
      selectHint: '请选择文档进行预览',
      searchPlaceholder: '搜索标题 / 文件名 / 分类',
      preview: '预览',
      ...
      ingest: '一键入库',
      ingestConfirm: '确认入库',
      ingestSuccess: '已入库知识库',
      kb: '目标知识库',
      kbPlaceholder: '选择知识库',
      cadPlaceholder: '暂无轻量化预览，可点击「重新转换」',
      cadConverting: '正在转换为轻量化模型…',
      cadConvert: '重新转换',
      cadPreviewFailed: '轻量化预览加载失败',
    },
    integration: {
      create: '新建对接',
      connectors: '对接配置',
