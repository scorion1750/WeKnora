export default {
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        title: 'ナレッジベースを作成',
        desc: 'すべてはここから始まります。ドキュメント、Webページ、FAQをアップロードすると、RAGが自動で解析してインデックスを作成します。ここをクリックしてナレッジベースを開いてください。'
      },
      chat: {
        title: 'AIチャットを開始',
  ...
  settings: {
    ...
    sandbox: {
      ...
      backendDescriptions: {
        ...
        docker: 'このRAGホスト上でセッションごとに長時間稼働するコンテナを保持します。スクリプトとファイルはそのコンテナ内に残ります',
      },
      ...
      dockerHostRisk: '空欄またはunix://を指定するとRAGホストのDockerデーモンを使用します。これはそのマシンのroot権限に相当します。単一ノードのプライベート環境でのみ使用してください。複数のワークスペースがホストを共有する場合はCubeまたはE2Bを推奨します。リモートのtcp://エンドポイントにはTLS証明書ディレクトリが必要です。',
      ...
      weknoraTemplateTitle: 'RAG標準テンプレート',
      weknoraDockerImage: 'RAG標準イメージ',
      weknoraDockerImageHint: 'セッションごとに専用の長時間稼働コンテナが割り当てられます。スクリプト、シェルコマンド、ファイルはすべてこのコンテナを共有し、セッション終了またはアイドルタイムアウトによる回収まで保持されます。',
      weknoraTemplateOverview: 'RAGが標準ランタイムを提供します。テンプレートは接続後に検出され、存在しない場合は標準テンプレートが作成されます。',
      weknoraTemplateDescription: 'RAGのスキルが必要とするPython、Node.js、CLIツール、ワークスペースパス、非root実行ユーザが含まれます。',
      recommendedTag: '推奨',
      ...
      standardTemplateProvisioning: 'RAG標準テンプレートを作成しています。しばらくしてから更新してステータスを確認してください。',
      ...
      templateUntaggedHint: 'ビルドは完了しましたが、デフォルトタグが付いたものがないため、サンドボックス作成時にこのテンプレートを解決できません。E2Bで削除して更新すると、RAGが再構築します。',
      templateFailedReason: 'ビルドに失敗しました: {reason}',
      noTemplates: 'このクラスタからテンプレートが返されませんでした。',
      weknoraStandardTemplate: 'RAG標準テンプレート',
      createStandardTemplate: '作成',
      createStandardTemplateHint: 'DNSを含む現在の接続設定でビルドされます。設定を変更した場合は、カードから再構築してください。',
      weknoraDesktopTemplate: 'RAGデスクトップテンプレート',
      ...
      replaceStandardTemplateConfirm: 'DNSを含む現在の設定でRAG標準テンプレートを再構築します。既存の起動可能なテンプレートは、新しいテンプレートの準備が整うまで削除されません。',
      desktopTemplateProvisioning: 'RAGデスクトップテンプレートを作成しています。しばらくしてから更新してステータスを確認してください。',
      desktopTemplateReplaced: '以前のデスクトップテンプレートを削除し、再構築を開始しました。完了までお待ちください。CLIテンプレートは変更しません。',
      desktopTemplateTag: 'デスクトップ',
      replaceDesktopTemplateConfirm: 'DNSを含む現在の設定でRAGデスクトップテンプレートを再構築します。既存の起動可能なデスクトップテンプレートは、新しいテンプレートの準備が整うまで削除されません。CLIテンプレートは変更しません。',
      ...
      cubeDnsServersHelp: '任意。RAG標準テンプレートに書き込まれるネームサーバのIPです。空欄の場合はクラスタのデフォルト（通常は119.29.29.29）を使用します。パブリックリゾルバへのUDP/53がブロックされている場合は、Cubeホストの/etc/resolv.confから到達可能なアドレス（10/8、172.16/12、192.168/16を除く）を指定してください。既存の標準テンプレートには、テンプレートカードで「再構築」を実行した後にのみ反映されます。',
      ...
      dockerTlsCertPathHelp: 'RAGホスト上でca.pem、cert.pem、key.pemを格納しているディレクトリです。リモートデーモンでは必須です。証明書はデプロイ側でマウントされ、ここには保存されません。',
      dockerIdleTtl: 'アイドル回収（秒）',
      dockerIdleTtlHelp: 'Dockerデーモン自体にはアイドルタイムアウトがありません。この時間コマンドが実行されなかったコンテナはRAGが回収し、セッションが続く場合は再作成します。空欄の場合は1800秒です。',
  ...
  envVarSettings: {
    ...
    description: 'スキルとサンドボックスで使う個人用のキーです。RAGのシステム設定やデプロイ設定ではありません。',
  ...
  createChat: {
    title: 'こんにちは、RAGです。あなたのナレッジを、すぐそばに',
  ...
  embedPublish: {
    ...
    originsHint: 'RAG の URL（B）ではなく、埋め込みを許可するホストサイト（A）を入力します。1 行に 1 つの Origin（例: https://shop.example.com）。1 件以上必須。*.example.com に対応。',
    ...
    webhookUrlDesc: '任意。RAGがmessage_sent / message_receivedイベントをこのHTTPSエンドポイントにPOSTします。',
  ...
  integrations: {
    ...
    api: {
      ...
      capabilityManageModelsHint: 'モデル定義、認証情報、接続確認、RAG Cloudの認証情報を管理します。',
      ...
      signedFlowDetail: 'HMACシークレットはRAGと信頼できる自社バックエンドにのみ保存し、リクエストに含めたりブラウザに配布したりしないでください。リクエストヘッダーで送るのは、そのシークレットで署名したJWT文字列（発行ごと、または有効期限切れ後に変わります）であり、シークレット自体ではありません。JWTにはsub（外部ユーザID）、tenant_id、aud=weknora、exp（有効期間は最大24時間）を含める必要があります。',
      ...
      hmacSecretDesc: 'RAGに保存されているものと同じシークレットです。JWTの署名のためバックエンドでのみ使用し、リクエストヘッダーとして送信しないでください。',
    ...
    chrome: {
      title: 'ナレッジアシスタント',
      subtitle: 'セルフホスト版RAG向け: ブラウジング中にサイドバーで質問したり、Webページをクリップしたり、Markdownメモをナレッジベースに保存したりできます。',
      ...
      steps: {
        ...
        port: {
          ...
          desc: 'RAGデスクトップ版では、API情報で固定ポート（例: 37841）を設定すると、再起動してもURLが変わりません。'
        },
        install: {
          title: '拡張機能をインストール',
    ...
    claw: {
      title: 'RAG Skill',
      subtitle: 'RAG REST API経由でドキュメントをインポートし、ハイブリッド検索（ベクトル＋キーワード）を実行します。ファイルのアップロード、URLインポート、Markdownでの登録、ナレッジベース横断検索に対応します。',
      ...
      installCtaHint: 'RAG Skillをインストール · 新しいタブで開きます',
