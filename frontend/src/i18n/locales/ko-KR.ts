export default {
  ...
  integrations: {
    ...
    claw: {
      title: 'RAG Skill',
      subtitle: 'RAG REST API로 문서를 가져오고 하이브리드 검색(벡터+키워드)을 수행합니다. 파일/URL/Markdown 업로드 및 검색에 사용.',
      ...
      installCtaHint: 'RAG Skill 설치 · 새 탭에서 열림',
    ...
    chrome: {
      title: '지식 관리 어시스턴트',
      subtitle: '자체 호스팅 RAG와 함께: 사이드바에서 질문하고, 웹 페이지를 클리핑하며, Markdown 메모를 지식베이스에 저장하세요.',
      ...
      steps: {
        ...
        port: {
          title: '데스크톱: 고정 포트(권장)',
          desc: 'RAG 데스크톱에서는 API 정보에서 고정 포트(예: 37841)를 설정하세요.'
        },
        api: {
          title: 'API 자격 증명',
    ...
    api: {
      ...
      capabilityManageModelsHint: '모델 설정, 자격 증명, 연결 테스트 및 RAG Cloud 자격 증명을 관리합니다.',
      ...
      signedFlowDetail: 'HMAC secret은 RAG와 신뢰할 수 있는 백엔드에만 보관하세요. 요청에 넣거나 브라우저에 배포하지 마세요. 요청 헤더에는 secret이 아니라 해당 secret으로 서명한 JWT 문자열을 넣습니다(발급마다 또는 만료 후 변경). JWT에는 sub(외부 사용자 ID), tenant_id, aud=weknora, exp(최대 24시간)가 필요합니다.',
      ...
      hmacSecretDesc: 'RAG에 저장된 것과 동일한 secret. 백엔드에서 JWT 서명에만 사용하며 요청 헤더로 보내지 않습니다.',
  ...
  embedPublish: {
    ...
    originsHint: 'RAG 주소(B)가 아닌 임베드를 허용할 호스트 웹사이트(A)를 입력하세요. 한 줄에 하나의 Origin(예: https://shop.example.com), 최소 1개. *.example.com 지원.',
  ...
  createChat: {
    title: '안녕하세요, RAG입니다 — 당신의 지식을 손끝에',
  ...
  auth: {
    ...
    registerSubtitle: '계정을 만들고 RAG를 시작하세요',
  ...
  envVarSettings: {
    title: '샌드박스 키',
    description: '스킬과 샌드박스에 쓰는 개인 키이며, RAG 시스템이나 배포 설정이 아닙니다.',
    helpAria: '샌드박스 키 설명',
  ...
  settings: {
    ...
    sandbox: {
      ...
      backendDescriptions: {
        ...
        docker: '이 RAG 호스트의 Docker에서 세션마다 장수명 컨테이너를 유지합니다. 스크립트와 파일이 같은 컨테이너에 남습니다',
      },
      ...
      dockerHostRisk: '비우거나 unix:// 를 쓰면 RAG가 있는 머신의 Docker 데몬을 사용하며, 그 권한은 해당 머신의 root와 같습니다. 프라이빗 단일 노드에만 쓰세요. 여러 워크스페이스가 같은 호스트를 쓰는 경우에는 Cube 또는 E2B를 쓰세요. 원격 tcp:// 는 TLS 인증서 디렉터리가 필요합니다.',
      addConfig: '샌드박스 추가',
      ...
      replaceStandardTemplateConfirm: '현재 설정(DNS 포함)으로 RAG 표준 템플릿을 다시 만듭니다. 새 템플릿이 준비되기 전에는 기존에 쓸 수 있는 템플릿을 삭제하지 않습니다.',
      desktopTemplateProvisioning: 'RAG 데스크톱 템플릿을 만들고 있습니다. 잠시 후 새로고침해 상태를 확인하세요.',
      desktopTemplateReplaced: '기존 데스크톱 템플릿을 삭제했고 재구축을 시작했습니다. 준비될 때까지 기다리세요. CLI 템플릿은 그대로 둡니다.',
      desktopTemplateTag: '데스크톱',
      replaceDesktopTemplateConfirm: '현재 설정(DNS 포함)으로 RAG 데스크톱 템플릿을 다시 만듭니다. 새 템플릿이 준비되기 전에는 기존에 쓸 수 있는 데스크톱 템플릿을 삭제하지 않으며, CLI 템플릿은 그대로 둡니다.',
      ...
      cubeDnsServersHelp: '선택 사항입니다. RAG 표준 템플릿에 기록되는 nameserver IP입니다. 비워 두면 클러스터 기본값(보통 119.29.29.29)을 사용합니다. 공용 DNS로 UDP/53이 나가지 않으면 Cube 호스트 /etc/resolv.conf에서 도달 가능한 주소를 넣고, 10/8, 172.16/12, 192.168/16은 피하세요. 이미 있는 표준 템플릿은 카드에서 「재구축」을 눌러야 변경이 적용됩니다.',
    ...
    weknoraCloud: {
      ...
      modelsSection: {
        title: '클라우드 모델',
        descReady: 'RAG Cloud의 네 가지 표준 모델을 등록하여 대화, 검색, 재정렬, 비전에 사용합니다.',
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        title: '지식 베이스 만들기',
        desc: '모든 것의 시작점입니다. 문서, 웹페이지, FAQ를 업로드하면 RAG가 자동으로 분석하고 색인합니다. 여기를 클릭해 지식 베이스로 이동하세요.'
      },
      welcome: {
        title: 'RAG에 오신 것을 환영합니다',
